...
class CategoryController extends Controller
{
    ...
    {
        try {
            $categories = $this->supabase->select('categories', ['*'], [], ['order' => 'name.asc']);

            foreach ($categories as &$category) {
                $recipes              = $this->supabase->select('recipes', ['id'], ['category_id' => $category['id'], 'status' => 'approved']);
     ...
     *
     * Schema: categories(id, name, slug NOT NULL UNIQUE, icon_url, created_at)
     * slug is auto-generated from name — must not be omitted or insert fails.
     */
    public function store(Request $request)
    {
        ...
        try {
            ...
            $existing = $this->supabase->select('categories', ['id'], ['slug' => $slug]);
            if (!empty($existing)) {
                return response()->json(['success' => false, 'message' => 'Category with this name already exists'], 400);
            }

            $category = $this->supabase->insert('categories', [
                'name'     => $name,
                'slug'     => $slug,           // NOT NULL UNIQUE — required
                'icon_url' => $request->input('icon_url'),
            ], true);

            return response()->json(['success' => true, 'message' => 'Category created successfully', 'data' => $category[0]], 201);
        } catch (Exception $e) {
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            if (!empty($recipes)) {
                return response()->json(['success' => false, 'message' => 'Cannot delete category with existing recipes'], 400);
            }
    ...
    public function recipes(Request $request, $id)
    {
        try {
            ...
            if (!in_array($orderBy, $allowedOrderBy, true)) $orderBy = 'created_at';

            $orderDirection = strtolower($request->input('order_direction', 'desc'));
            if (!in_array($orderDirection, ['asc', 'desc'], true)) $orderDirection = 'desc';

            $recipes = $this->supabase->select('recipes',
                ['*', 'profiles(*)', 'recipe_tags(tags(*))'],
                ['category_id' => $id, 'status' => 'approved'],
                ['order' => "{$orderBy}.{$orderDirection}", 'limit' => $limit, 'offset' => $offset]
            );
