...
<head>
    ...
    <style>
        .msg-user { background: var(--gradient-accent); }
...
<body class="bg-[#F5F7FA] text-gray-900 flex flex-col h-screen">
    ...
    <header class="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm flex-shrink-0">
        <div class="max-w-3xl mx-auto px-4 h-14 flex items-center gap-3">
            ...
            <div class="w-8 h-8 bg-gradient-accent rounded-full flex items-center justify-center flex-shrink-0">
                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/>
                </svg>
            </div>
            <div class="flex-1 min-w-0">
                <p class="font-bold text-sm truncate" style="color: var(--color-text-primary)">{{ $conversation['title'] ?? 'Chef AI' }}</p>
                <p class="text-xs" style="color: var(--color-text-secondary)">Tap untuk ganti judul</p>
            </div>
            <a href="{{ route('app.ai.settings') }}" class="p-2 rounded-full hover:bg-gray-100 transition-colors" style="color: var(--color-text-secondary)">
                ...
                <div class="flex items-end gap-3">
                    <div class="flex-1 bg-gray-100 rounded-2xl px-4 py-3">
                        <textarea name="content" rows="1" placeholder="Tanya tentang masak..."
                                  class="w-full bg-transparent text-sm placeholder-gray-400 resize-none focus:outline-none"
                                  style="max-height: 120px; color: var(--color-text-primary)"
                                  onInput="this.style.height='auto'; this.style.height=this.scrollHeight+'px'"></textarea>
                    </div>
                    <button type="submit"
                            class="btn-primary-savora w-11 h-11 rounded-full flex-shrink-0 p-0">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
