    ...
    <div class="max-w-3xl mx-auto px-4 py-6 pb-24 md:pb-10">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-4">
                <div class="p-3 bg-gradient-accent rounded-2xl shadow-lg">
            ...
            <div class="flex gap-2">
                <a href="{{ route('app.ai.settings') }}"
                   ...
                   onmouseover="this.style.color='var(--color-primary-coral)'; this.style.borderColor='rgba(231,111,81,0.3)'"
                   onmouseout="this.style.color='var(--color-text-secondary)'; this.style.borderColor=''">
        ...
        <form action="{{ route('app.ai.create') }}" method="POST" class="mb-6">
            @csrf
            <button type="submit" class="btn-primary-savora w-full py-4 rounded-2xl text-lg gap-3">
                    ...
                    <a href="{{ route('app.ai.conversation', $conv['id']) }}" class="flex items-center gap-4 p-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between gap-2 mb-1">
                                <p class="font-bold truncate text-sm" style="color: var(--color-text-primary)">{{ $conv['title'] ?? 'New Chat' }}</p>
                                <span class="text-xs flex-shrink-0" style="color: var(--color-text-secondary)">{{ $timeAgo }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="px-2 py-0.5 text-xs font-bold rounded-full {{ $providerColor }}">{{ $providerLabel }}</span>
                            </div>
                        </div>
                        <svg class="w-4 h-4 flex-shrink-0" style="color: var(--color-text-secondary)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
