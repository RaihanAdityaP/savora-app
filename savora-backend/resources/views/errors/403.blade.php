...
<head>
    ...
    <title>{{ $isEnglish ? '403 - Access Denied' : '403 - Akses Ditolak' }} | Savora</title>
...
<body>
    ...
    <div class="error-card">
        ...
        <h1 class="error-title">{{ $isEnglish ? 'Access Denied' : 'Akses Ditolak' }}</h1>
        <p class="error-subtitle">
            {{ $isEnglish ? 'You do not have permission to access this page.' : 'Kamu tidak memiliki izin untuk mengakses halaman ini.' }}<br>
            {{ $isEnglish ? 'Please log in first or contact admin if you think this is a mistake.' : 'Silakan login terlebih dahulu atau hubungi admin jika kamu yakin ini adalah kesalahan.' }}
        </p>
        ...
        <div class="error-actions">
            ...
            @guest
                <a href="{{ route('app.login') ?? '/login' }}" class="btn-login">
                    ...
                    {{ $isEnglish ? 'Log In Now' : 'Login Sekarang' }}
                </a>
            @else
                <a href="{{ route('app.home') ?? '/' }}" class="btn-login">
                    <i class="bi bi-house-fill"></i>
                    {{ $isEnglish ? 'Go Home' : 'Ke Beranda' }}
                </a>
            @endguest
        </div>
    </div>
