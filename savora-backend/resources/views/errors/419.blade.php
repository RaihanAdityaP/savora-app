...
<head>
    ...
    <title>{{ $isEnglish ? '419 - Session Expired' : '419 - Sesi Kedaluwarsa' }} | Savora</title>
...
<body>
    ...
    <div class="error-card">
        ...
        <h1 class="error-title">{{ $isEnglish ? 'Session Expired' : 'Sesi Kedaluwarsa' }}</h1>
        <p class="error-subtitle">
            {{ $isEnglish ? 'Your session has expired. This can happen when a tab is left open for too long' : 'Sesi kamu sudah habis masa berlakunya. Ini terjadi jika tab dibiarkan terlalu lama' }}<br>
            {{ $isEnglish ? 'or the security token is no longer valid. Reload the page to continue.' : 'atau token keamanan tidak valid. Muat ulang halaman untuk melanjutkan.' }}
        </p>

        <div class="error-detail-box">
            <i class="bi bi-shield-exclamation"></i>
            <p>{{ $isEnglish ? 'The CSRF token is invalid. Make sure cookies are enabled in your browser, then reload the page.' : 'Token CSRF tidak valid. Pastikan cookies diaktifkan di browser kamu, lalu coba muat ulang halaman.' }}</p>
        </div>
